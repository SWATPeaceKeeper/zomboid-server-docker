...
- **Images are signed, and carry provenance and an SBOM.** The signature is
  keyless and bound to this repository's workflow identity, so there is no
  private key to leak and the claim "this came from that commit in this
  repository" is verifiable rather than asserted. Applies from 2.0.0 onwards;
  1.2.1 and earlier were built before the pipeline signed anything. The `cosign
  verify` command is in the README under Images and versioning.
